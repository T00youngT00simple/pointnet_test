...
g_classes = [x.rstrip() for x in open(os.path.join(ROOT_DIR, 'data_preparation/meta/class_names.txt'))]
g_class2label = {cls: i for i,cls in enumerate(g_classes)}
g_class2color = {'ground':	[0,0,255],     # blue
                 'vegetation':	[0,255,0],   # green
                 'building':	[0,255,255],   # cyan
                 'clutter':        [255,0,0]}  # red
...
def collect_point_label(anno_path, out_filename, file_format='txt'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):
        ...
        points_list.append(np.concatenate([points, rgb3col, labels], 1))   # Nx(6+1)
